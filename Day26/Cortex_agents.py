...
USE DATABASE "{db_name}"; USE SCHEMA "{schema_name}"; USE WAREHOUSE COMPUTE_WH;"""
    ...
    if st.button(":material/play_arrow: Run Step 1", key="run_step1", use_container_width=True):
        with st.spinner("Creating database and schema..."):
            try:
                for sql in [f'CREATE OR REPLACE DATABASE "{db_name}"',
            ...
            try:
                ...
                try:
                    existing = session.sql(
                        f'SHOW CORTEX SEARCH SERVICES IN SCHEMA "{db_name}"."{schema_name}"').collect()
                ...
                if service_exists:
                    ...
                    session.sql(
                        f'ALTER TABLE "{db_name}"."{schema_name}".SALES_CONVERSATIONS SET CHANGE_TRACKING = TRUE').collect()
                    ...
                    status.update(label="✓ Step 3 complete! Service is indexing in background (1-2 min)",
                                  state="complete")
            except Exception as e:
                st.error(f"Error: {e}")
...
"""
    ...
    with col2:
        if st.button(":material/cloud_upload: Auto-Upload to Stage", key="run_step5", use_container_width=True,
                     type="primary"):
            with st.spinner("Creating stage and uploading YAML..."):
                try:
                    ...
                    try:
                        ...
                        session.file.put(temp_file_path, f'@"{db_name}"."{schema_name}".MODELS',
                        ...
                        if 'sales_metrics_model.yaml' in uploaded_files:
                            ...
                            found_file = next((f for f in uploaded_files if 'sales_metrics_model' in f.lower()),
                                              None)
                            if found_file:
                                st.warning(
    ...
    if st.button(":material/verified: Check if Data is Ready", type="primary", use_container_width=True):
        with st.status("Verifying setup...", expanded=True) as status:
            ...
            checks = [
                ...
                (f'SELECT COUNT(*) as cnt FROM "{db_name}"."{schema_name}".SALES_CONVERSATIONS', "Conversations table",
                 True),
                (f'SHOW CORTEX SEARCH SERVICES IN SCHEMA "{db_name}"."{schema_name}"', "Cortex Search service", False,
                 search_service),
                (f'SELECT COUNT(*) as cnt FROM "{db_name}"."{schema_name}".SALES_METRICS', "Sales metrics table", True,
                 None, True),
            ...
            if all_good:
                status.update(label=":material/celebration: Day 27 data ready! (Day 28+ data optional)",
                              state="complete")
                st.balloons()
            else:
